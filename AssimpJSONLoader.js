...
export default function AssimpJSONLoader ( manager ) {
...
AssimpJSONLoader.prototype = {
	...
	parse: function ( json, path ) {
		...
		function parseMesh( json ) {
			...
			// We need to highlight faces so buffer geometry is not going to work.
		...
		function parseObject( json, node, meshes, materials ) {
			...
			for ( i = 0; node.meshes && i < node.meshes.length; i ++ ) {
				...
				var mat = new THREE.MeshPhongMaterial({color: COLORS.OBJECT_BASE_COLOR });
				mat.vertexColors = THREE.FaceColors;
				var mesh = new THREE.Mesh( meshes[ idx ], mat);
		...
		// We want to define the material, we don't care what the model wants to look like.
